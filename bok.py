...
class bokehPlots():
    ...
    def plot_line(self):
        ...
        for ranking in ['by top avg', 'by top cluster', 'by bayes mi', 'by random', 'by bottom avg',
            ...
            inter_types = ['ablation', r'$\alpha=2$',
                           ...
                           r'ranking scale $\alpha=12$', r'ln scale $\alpha=6$', r'ln scale $\alpha=8$',
                           r'ln scale $\alpha=12$']
        ...
        num_ablated = [str(r[0]) for r in self.res['by top avg']['wrong words']]
        self.plot_by_plt(num_ablated, all_rankings_res, 'all', False)
        max_points = {ranking: self.find_knee_point(stats[1], 1.05, num_ablated) for ranking, stats in all_rankings_res.items()}
        return max_points

    def find_knee_point(self, stats:np.array, k, x_ticks):
        for i in range(len(stats)-2):
            improvement1, improvement2 = stats[i+1] / stats[i], stats[i+2] / stats[i]
            if improvement1 < k and improvement2 < k:
    ...
    def plot_by_plt(self, x_ticks, stats, ranking, within_ranking):
        ...
        for name, (wrong_words, clwv) in stats.items():
            ...
            line, = ax.plot(x_ticks[:len(wrong_words)], clwv, color=colors[name], label=name,
...
def create_dataset(languages):
    ...
    with open(Path('results', 'UM', 'max_clwv_lnscale.pkl'), 'rb') as f:
        res = pickle.load(f)
        ...
        for att, a_res in l_res.items():
            ...
            for layer, layer_res in a_res.items():
                for ranking, r_res in layer_res.items():
                    df[(lan, att, layer, ranking)]['max'] = round(r_res['max'],2)
                    df[(lan, att, layer, ranking)]['argmax'] = r_res['argmax']
    ...
    for lan in languages:
        for att in attributes:
            relevant_data = df.loc[idx[['max', 'argmax']], idx[[lan], [att], layers, rankings]]
            print(f'{att}:')
            print('absolute:')
