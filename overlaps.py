...
def plot_bar(data: dict, settings, num_to_show, layer, save_dir):
    ...
    order = [settings.index('Number, fin'), settings.index('POS, fra'), settings.index('Gender, hin'),
             settings.index('Voice, rus'),
             settings.index('Number, rus'), settings.index('POS, ara'), settings.index('POS, bul'),
             settings.index('Aspect, rus')]
    fig, ax = plt.subplots()
